...
class OrderHeader(base_models.AuditModel):
    ...
    depo_code = models.ForeignKey(DepoMaster, db_column='DEPOCODE')
    customer_reference_number = models.CharField(max_length=150, null=True, blank=True, db_column='CUSREFNO')
...
class OrderDetails(base_models.AuditModel):
    ...
    order_date = models.DateField(db_column='OPDT')
    product_code = models.ForeignKey(ProductMaster, db_column='PRODCODE')
    order_quantity = models.FloatField(db_column='ORDQTY')
    adjust_quantity = models.FloatField(null=True, blank=True, db_column='ADJQTY')
    ...
    all_quantity = models.FloatField(null=True, blank=True, db_column='ALLQTY')
    sent_quantity = models.FloatField(null=True, blank=True, db_column='SENTQTY')
    hold_quantity = models.FloatField(null=True, blank=True, db_column='HOQTY')
    ...
    ware_house_qty = models.FloatField(null=True, blank=True, db_column='WH_TRFQTY')
...
class InvoiceHeader(base_models.AuditModel):
    ...
    roundoff = models.FloatField(db_column='ROUNDEDOFF', default=0)
    ...
    net_amount = models.FloatField(db_column='NETAMT', default=0)
    ...
    total_cgst_amount = models.FloatField(db_column='TOT_CGSTAMT', default=0)
    total_sgst_amount = models.FloatField(db_column='TOT_SGSTAMT', default=0)
    total_igst_amount = models.FloatField(db_column='TOT_IGSTAMT', default=0)
    invoice_gstin = models.CharField(max_length=15, db_column='INV_GSTIN', null=True, blank=True)
    close_flag = models.IntegerField(db_column='CLOSEFLAG', default=0)
    bouns_flag = models.IntegerField(db_column='BOUNSFLAG', default=0)
    grn_flag = models.IntegerField(db_column='GRNFLAG', default=0)
    tot_grnvalue = models.IntegerField(db_column='TOT_GRNVALUE', default=0)
    ...
    def to_json(self):
        return {
            ...
            "cancel": str(self.cancel or ""),
            "depo": str(self.depo_code.depo_code),
            "bank_code": str(self.bank_code or ""),
            "bank_document_date": str(self.bank_document_date or ""),
            "customer": str(self.customer_code.customer_code or self.customer_code.sfa_temp_id),
            "balance_amount": str(self.balance_amount or 0),
            "total": str(self.total or 0),
            "total_discount": str(self.total_discount or 0),
            "remarks": self.remarks or "",
            "roundoff": str(self.roundoff or 0),
            "total_product_value": str(self.total_product_value or 0),
            "total_discount_value": str(self.total_discount_value or 0),
            "total_tax_value": str(self.total_tax_value or 0),
            "total_tax_amount": str(self.total_tax_amount or 0),
            "net_amount": str(self.net_amount or 0),
            "advance_amount": str(self.advance_amount or 0),
            "credit_amount": str(self.credit_amount or 0),
            "dr_amount": str(self.dr_amount or 0),
            "paid_amount": str(self.paid_amount or 0),
            "invoice_amount": str(self.invoice_amount or 0),
            "other_charges": str(self.other_charges or 0),
            "gross_weight": str(self.gross_weight or 0),
            "total_cgst_amount": str(self.total_cgst_amount or 0),
...
class InvoiceDetails(base_models.AuditModel):
    ...
    def to_json(self):
        return {
            ...
            'order_date': str(self.order_date),
            'total': str(self.total or 0),
            'cgst_per': str(self.cgst_per or 0),
            'cgst_amount': str(self.cgst_amount),
            'sgst_per': str(self.sgst_per or 0),
            'sgst_amount': str(self.sgst_amount),
            'igst_per': str(self.igst_per or 0),
...
class PaymentAdjustmentDetails(base_models.AuditModel):
    id = models.AutoField(primary_key=True, db_column='ID')
    invoice_no = models.CharField(null=False,db_column='INVNO', max_length=15, default=0)
    invoice_date = models.DateField(db_column='INVDT', null=True, blank=True,)
    ...
    doc_number = models.IntegerField(null=True, blank=True, db_column='DOCNUM')
    doc_date = models.DateField(null=True, blank=True, db_column='DOCDT')
    ...
    def to_json(self):
        return {
            ...
            'customer_code': str(self.customer_code or ""),
            'doc_number': str(self.doc_number or ""),
            'doc_date': str(self.doc_date or ""),
